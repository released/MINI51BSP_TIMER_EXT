...
#define FLAG_PROJ_PWM_INC                   			(flag_PROJ_CTL.bit1)
#define FLAG_PROJ_PWM_DEC                 				(flag_PROJ_CTL.bit2)
...
{
    uint32_t t = TIMER_GetCaptureData(TIMER1);

    // Timer clock source us 22.1184M
    // printf("Input frequency is %dHz\r\n", 22118400 / t);
    freq_detect = 22118400 / t;

    TIMER_ClearCaptureIntFlag(TIMER1);
}

void TIMER_CAP_Init(void)   // use P3.6 to capture input signal ( 100Hz ~ 1000Hz)
{
    ...
    // Configure Timer 1 free counting mode, capture TDR value on rising edge
    TIMER_EnableCapture(TIMER1, TIMER_CAPTURE_TRIGGER_COUNTING_MODE, TIMER_CAPTURE_RISING_EDGE);
...
void PWM_SET_FREQ(uint32_t freq)   // use PWM0 (P2.2) to simulate input signal ( 100Hz ~ 1000Hz)
{
    PWM_ConfigOutputChannel(PWM,0,freq,50);
...
void loop(void)
{
    ...
    if (FLAG_PROJ_PWM_INC)
    {
        FLAG_PROJ_PWM_INC = 0;  
        freq_target = (freq_target >= 1000) ? (1000) : (freq_target+10) ;      
        PWM_SET_FREQ(freq_target);
        printf("freq(INC) :%3d\r\n" , freq_target);

        SysTick_delay(10);
        printf("detect freq :%3d\r\n" , freq_detect);
    }

    if (FLAG_PROJ_PWM_DEC)
    {
        FLAG_PROJ_PWM_DEC = 0;        
        freq_target = (freq_target <= 50) ? (50) : (freq_target-10) ;
        PWM_SET_FREQ(freq_target);
        printf("freq(DEC) :%3d\r\n" , freq_target);
